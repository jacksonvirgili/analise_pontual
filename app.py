...
Original file is located at
    https://colab.research.google.com/drive/1iSG57HPCe8ezQi_cyUvPUq7yt2lvWcd0
"""
...
lista_subgrupos = df_resumo['Material - Subgrupo'].unique().tolist()
...
layout_grafico = st.sidebar.radio("Layout das Barras:", options=['Lado a Lado', 'Empilhado'])


# 3. LÓGICA DO GRÁFICO
# --- FILTRO ---
df_f = df_resumo[df_resumo['Material - Subgrupo'] == subgrupo].copy()
if classificacao != 'Todas':
    df_f = df_f[df_f['Classificação Preço'] == classificacao]

# --- VERIFICAÇÃO DE DADOS ---
if df_f.empty:
    # Mostra um aviso bonito na tela do Streamlit
    st.warning(f"Não houve vendas de '{subgrupo}' na classificação '{classificacao}'.")
else:
    # --- AGRUPAMENTOS ---
    ...
    # Define o modo das barras dinamicamente
    modo_barras = 'group' if layout_grafico == 'Lado a Lado' else 'stack'

    # Layout básico
    fig.update_layout(
        title=f"Faturamento | {subgrupo} - {classificacao.upper()}",
        barmode=modo_barras,
        hovermode="x unified",
    ...
    # use_container_width=True faz o gráfico se adaptar ao tamanho da tela!
